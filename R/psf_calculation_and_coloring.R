...
visIgraphLayout_new <- function (graph, layout = "layout_nicely", physics = FALSE, smooth = FALSE, 
                                 type = "square", randomSeed = NULL, layoutMatrix = NULL,
                                 ...) {
  ...
  ig <- igraph::graph_from_data_frame(graph$x$edges[, c("from", 
                                                        "to")], directed = TRUE, vertices = graph$x$nodes[, c("id", 
                                                                                                              setdiff(names(graph$x$nodes), "id"))])
  ...
  graph %>% visNodes(physics = physics) %>% visEdges(smooth = smooth) %>% 
