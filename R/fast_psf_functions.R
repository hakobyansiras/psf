...
#' @param seurat_object Seurat object of spatial transcriptomics dataset.
...
interactive_spatial_plot <- function(seurat_object, feature = NULL, group = "seurat_clusters", log_norm = T) {
  ...
  spatial_image <- as.raster(seurat_object@images$slice1@image)
  
  # Extract the low-resolution coordinates using GetTissueCoordinates
  coords <- GetTissueCoordinates(object = seurat_object, scale = "lowres")
  
  coords$imagerow_flipped <- nrow(spatial_image) - coords$imagerow
  
  # Define the image bounds based on coordinates (no padding)
  x_min_image <- 0
  ...
  if(is.null(feature)) {
    ...
    # Extract spot cluster information
    spot_groups <- seurat_object@meta.data[,group] 
    
    # Create a plot using Plotly with clusters as color groups
    p <- plot_ly(x = coords$imagecol, y = coords$imagerow_flipped,  
                 type = 'scatter', mode = 'markers',
                 text = paste(rownames(coords), '\n', 'Cluster:', spot_groups),  # Hover text displays cluster information
                 hoverinfo = 'text',
                 color = spot_groups,  # Color spots by clusters
                 colors = gg_color_hue(length(unique(spot_groups))),
                 marker = list(size = 6)) %>%
      layout(
        images = list(
          list(
            source = raster2uri(spatial_image),  # Add the spatial image as a background
            xref = "x",  # Align image with the x-axis (using spot grid positions)
            yref = "y",  # Align image with the y-axis (using spot grid positions)
            x = x_min_image,  # Set the minimum x position of the image
            y = y_max_image,  # Set the maximum y position of the image
            sizex = (x_max_image - x_min_image),  # Image width
            sizey = (y_max_image - y_min_image),  # Image height
            sizing = "stretch",  # Stretch the image to fit within the scatter plot area
            opacity = 1,  # Adjust transparency of the image
            layer = "below"  # Place the image behind the scatterplot
          )
        ),
        xaxis = list(# title = 'X-coordinates', 
          range = c(x_min_image, x_max_image), scaleanchor = "y", showgrid = FALSE, zeroline = FALSE, showticklabels=FALSE),
        yaxis = list(# title = 'Y-coordinates', 
          range = c(y_min_image, y_max_image), scaleanchor = "x", showgrid = FALSE, zeroline = FALSE, showticklabels=FALSE),
        showlegend = TRUE,  # Show legend to indicate clusters
    ...
    feature_data <- FetchData(object = seurat_object, vars = feature, layer = "counts")
    if(log_norm) {
      feature_data <- log(feature_data)
    }
    
    p <- plot_ly(x = coords$imagecol, y = coords$imagerow_flipped,  # Use imagecol and imagerow_flipped
                 type = 'scatter', mode = 'markers',
                 text = paste(rownames(feature_data), '\n', 'Value:', round(feature_data[,feature], 2)),  # Hover text shows the continuous value
                 hoverinfo = 'text',
                 marker = list(size = 6, color = feature_data[,feature], colorscale = 'Viridis', showscale = TRUE)) %>%
      layout(
        images = list(
          list(
            source = raster2uri(spatial_image),  # Add the spatial image as a background
