...
#' @import grDevices
#' @export
calc_psf_and_generate_report_from_collection <- function(kegg_collection, exp_matrix, file_name) {
  ...
  plot_list <- lapply(names(kegg_collection), function(x) {
    ...
    pathway_img_new <- kegg_designer(group_graphics = kegg_collection[[x]]$group_nodes, node_graphics = graphical_data$node_coords, 
                                     pathway_image = magick::image_read(system.file("extdata", "pathway_imgs", paste0(gsub("path:", "", kegg_collection[[x]]$attrs$name), ".png"), package="psf")),
