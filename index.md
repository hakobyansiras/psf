...
The **PSF Toolkit** is an R package developed for **topology-aware (TA)
pathway analysis** of various types of omics data. The package includes
...
> **Note:** Due to potential network issues with GitHub servers, the
> package download may take longer than expected. If you encounter
> timeout errors, try increasing the timeout limit using:
>
> ``` r
> options(timeout = 400)
> ```
...
The PSF Toolkit includes a **Shiny** app for interactive pathway
...
A detailed tutorial on pathway import, curation, and analysis is
...
> **Important:** Due to installation issues with the `Seurat` package,
> it is included in `Suggests` rather than `Imports`. The PSF package
> will still install even if `Seurat` fails to install. However, you
> must manually install `Seurat` to perform spatial transcriptomic
> analyses.
...
A short demo video demonstrating the PSF spatial browser is available:
[PSF Spatial Browser Demo](https://www.youtube.com/watch?v=lHTgYBA374o)

You can also explore a **demo Shiny app**: [PSF Spatial
Browser](https://apps.armlifebank.am/PSF_spatial_browser/)

The app includes a **dedicated Help tab** containing detailed
...
For more details, please refer to the package documentation and
tutorials.
